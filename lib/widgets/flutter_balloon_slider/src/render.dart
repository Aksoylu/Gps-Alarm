...
import '../flutter_balloon_slider.dart';
...
class BalloonSliderWidget extends LeafRenderObjectWidget {
  ...
  void updateRenderObject(BuildContext context,
      _BalloonSliderRender renderObject) {
    renderObject
      ..percent = percent
      ..maxValue = maxValue
      ..ropeLength = ropeLength
...
class _BalloonSliderRender extends RenderBox {
  _BalloonSliderRender({required double percent,
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    //draw progress
    Rect progressRect = _getTrackRect(offset: offset, progress: _percent);
    ...
    Rect thumbRect = _getThumbRect(
        offset: Offset(
            trackRect.left + _percent * trackRect.width, trackRect.center.dy));
    ...
    if (_balloonPath == null) {
      ...
      _balloonPath = Path()
        ..arcTo(
            ...
            false)
        ..quadraticBezierTo(
            -balloonRect.width / 2, 8, -3,
            balloonRect.height / 2 - 5)..quadraticBezierTo(
            0, balloonRect.height / 2 + 5, 3,
            balloonRect.height / 2 - 5)..quadraticBezierTo(
    ...
    //draw text
    if (_oldValue != _percent) {
      final val = (_percent * maxValue).round();
      _textPainter
        ..text = TextSpan(
            text: "$val",
  ...
  void _onDragStart(DragStartDetails details) {
    if (!_active) {
      if (onChangeStart != null) onChangeStart!(_percent);

      Rect _trackRect = _getTrackRect();
      _currentDragValue =
      ((globalToLocal(details.globalPosition).dx - _trackRect.left) /
          _trackRect.width);
      _percent = _currentDragValue.clamp(0.0, 1.0);
      onChanged(_percent);
  ...
  void _handleDragEnd() {
    if (_active && _state.mounted) {
      if (onChangeEnd != null) onChangeEnd!(_percent);
  ...
  Rect _getBalloonRect({Offset offset = Offset.zero,
